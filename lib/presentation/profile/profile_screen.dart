...
class ProfileScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: SafeArea(child: SingleChildScrollView(child: Column(children: [
      ...
      Container(color: AppColors.white, padding: const EdgeInsets.fromLTRB(16, 20, 16, 20), child: Row(children: [
        ...
        IconButton(icon: const Icon(Icons.edit_outlined, color: AppColors.primary, size: 20), onPressed: () => navigator<NavigationService>().navigateTo(AppRoutes.account)),
      ])),
      ...
      Padding(padding: const EdgeInsets.fromLTRB(16, 8, 16, 8), child: Column(children: [
        _Section(title: 'Account', items: [
          _Item(icon: Icons.person_outline,      label: 'Account Details',        onTap: () => navigator<NavigationService>().navigateTo(AppRoutes.account)),
          _Item(icon: Icons.language_outlined,   label: 'Language',               onTap: () => navigator<NavigationService>().navigateTo(AppRoutes.language)),
          _Item(icon: Icons.notifications_outlined, label: 'Notification Settings', onTap: () => navigator<NavigationService>().navigateTo(AppRoutes.notificationSettings)),
        ]),
        const SizedBox(height: 12),
        _Section(title: 'Support & Legal', items: [
          _Item(icon: Icons.help_outline,        label: 'Help & Support',         onTap: () {}),
          _Item(icon: Icons.description_outlined, label: 'Terms of Service',      onTap: () => navigator<NavigationService>().navigateTo(AppRoutes.terms)),
          _Item(icon: Icons.privacy_tip_outlined, label: 'Privacy Policy',        onTap: () => navigator<NavigationService>().navigateTo(AppRoutes.privacy)),
  ...
  void _confirmLogout(BuildContext ctx) => showDialog(context: ctx, builder: (_) => AlertDialog(
    ...
    actions: [
      ...
      TextButton(onPressed: () { Navigator.pop(ctx); ctx.read<AuthBloc>().add(AuthSignOut()); }, child: Text('Sign Out', style: T.body.copyWith(color: AppColors.error))),
    ],
  ));
}
