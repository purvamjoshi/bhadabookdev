...
class _State extends State<OtpScreen> {
  ...
  void _verify() {
    if (!_full) return;
    final vid = 'mock-vid-${widget.phone.replaceAll(RegExp(r'[^\d]'), '')}';
    context.read<AuthBloc>().add(AuthVerifyOtp(vid, _otp));
  }
  ...
  Widget build(BuildContext context) => BlocListener<AuthBloc, AuthState>(
    ...
    child: Scaffold(backgroundColor: AppColors.white,
      appBar: AppBar(backgroundColor: AppColors.white, elevation: 0, leading: IconButton(icon: const Icon(Icons.arrow_back_ios_new_rounded, size: 20), onPressed: () => navigator<NavigationService>().goBack())),
