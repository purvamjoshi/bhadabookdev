...
class _Shimmer extends StatelessWidget {
  const _Shimmer();
  @override Widget build(BuildContext context) => Padding(padding: S.pageAll, child: const Column(children: [
...
class _TopBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(color: AppColors.white, padding: const EdgeInsets.fromLTRB(16,12,16,12), child: Row(children: [
    ...
    IconButton(icon: const Icon(Icons.notifications_outlined, color: AppColors.navy), onPressed: () => navigator<NavigationService>().navigateTo(AppRoutes.notifications)),
  ]));
}
...
class _SetupCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => BBCard(child: Row(children: [
    ...
    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      GestureDetector(onTap: () => navigator<NavigationService>().navigateTo(AppRoutes.addProperty), child: Text('CONTINUE SETTING UP →', style: T.caption.copyWith(color: AppColors.primary, fontWeight: FontWeight.w700, letterSpacing: 0.5))),
    ])),
  ]));
}
...
class _PayCard extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => BBCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    const SizedBox(height: 12),
    if (p.isPaid)
      BBButton.secondary(label: 'VIEW RECEIPT', height: S.btnSmH, onPressed: () => navigator<NavigationService>().navigateTo(AppRoutes.receipt, arguments: p))
    else
      BBButton.sm(label: 'MARK AS PAID', onPressed: () => navigator<NavigationService>().navigateTo(AppRoutes.markPaid, arguments: p)),
  ]));
}
