  ...
  Widget build(BuildContext context) => BlocBuilder<PropertyBloc, PropertyState>(builder: (ctx, s) => Scaffold(
    ...
    body: SafeArea(child: Column(children: [
      ...
      Container(color: AppColors.white, padding: const EdgeInsets.fromLTRB(16,16,16,12), child: Row(children: [
        ...
        BBButton.sm(label: 'Add Property', icon: Icons.add, fullWidth: false, onPressed: () => context.push('/properties/add')),
      ])),
      ...
      Expanded(child: s.loading
        ...
        : s.current.isEmpty
          ? BBEmptyState(icon: Icons.apartment_outlined, title: 'No Properties Yet', desc: 'Add your first property to start tracking rent and managing tenants.', ctaLabel: 'Add Property', onCta: () => ctx.push('/properties/add'))
...
class _PropCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => BBCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    if (!isOccupied)
      BBButton.secondary(label: 'ADD AGREEMENT', height: S.btnSmH, onPressed: () => context.push('/agreement/step1', extra: {'propertyId': p.id}))
    else
