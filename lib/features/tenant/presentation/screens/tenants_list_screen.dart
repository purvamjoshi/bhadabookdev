  ...
  Widget build(BuildContext context) => BlocBuilder<TenantBloc, TenantState>(builder: (ctx, s) => Scaffold(
    ...
    body: SafeArea(child: Column(children: [
      Container(color: AppColors.white, padding: const EdgeInsets.fromLTRB(16, 16, 16, 12), child: Row(children: [
        ...
        BBButton.sm(label: 'Add Tenant', icon: Icons.add, fullWidth: false, onPressed: () => ctx.push('/tenants/add')),
      ])),
      const SizedBox(height: 8),
      Expanded(child: s.loading
        ? const Padding(padding: S.pageAll, child: BBListShimmer())
        : s.tenants.isEmpty
          ? BBEmptyState(icon: Icons.people_outline, title: 'No Tenants Yet', desc: 'Your tenants will appear here once added.', ctaLabel: 'Add Tenant', onCta: () => ctx.push('/tenants/add'))
          : ListView.builder(
...
class _TenantCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => BBCard(child: Row(children: [
    ...
    IconButton(
      ...
      onPressed: () => context.push('/tenants/${tenant.id}'),
    ),
  ]));
}
