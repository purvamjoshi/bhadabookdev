...
class _State extends State<PhoneInputScreen> {
  ...
  Widget build(BuildContext context) => BlocListener<AuthBloc, AuthState>(
    listener: (ctx, s) {
      if (s is AuthOtpSent) ctx.go('/otp', extra: {'phone': s.phone, 'vid': s.vid});
      if (s is AuthError) setState(() => _err = s.msg);
    },
    child: Scaffold(backgroundColor: AppColors.white,
      appBar: AppBar(backgroundColor: AppColors.white, elevation: 0, leading: IconButton(icon: const Icon(Icons.arrow_back_ios_new_rounded, size: 20), onPressed: () => context.go('/welcome'))),
