...
class _State extends State<OnboardingStep2Screen> {
  ...
  Widget build(BuildContext context) => BlocListener<OnboardingBloc, OnboardingState>(
    listener: (ctx, s) { if (s is OnboardStep2) ctx.go('/onboarding/step3'); },
    child: Scaffold(backgroundColor: AppColors.white,
      appBar: AppBar(backgroundColor: AppColors.white, elevation: 0,
        leading: IconButton(icon: const Icon(Icons.arrow_back_ios_new_rounded, size: 20), onPressed: () => context.go('/onboarding/step1')),
      ...
      body: SafeArea(child: SingleChildScrollView(padding: S.pageAll, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          Expanded(child: BBButton.secondary(label: '← BACK', onPressed: () => context.go('/onboarding/step1'))),
          const SizedBox(width: 12),
